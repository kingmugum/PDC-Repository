...
- Governance 문서만 변경되고 승인 기능 Requirement와 Code가 변하지 않은 경우 Baseline Revision은 올리지 않는다. Baseline 식별 자체를 의도적으로 변경해야 할 때에만 기능 로직 변경 없이 revision rollover를 수행한다.
...
- 기준일: 2026-09-12
- Baseline: `rev87`
- 기준 배포: `PassFail_260912_1.zip`
- 요구사항 구조: 사용자 시나리오 + AI_01~AI_10
- 다음 신규 FR 참고값: `FR-160`

### C1.11 기존 분리 Governance 대체 및 자동 준수 계약

이 프로필은 기존 PassFail 패키지의 다음 두 문서를 대체한다.

- `00_AI_DEVELOPMENT_CONSTITUTION.md` → 본 문서 `Part A`
- `01_PROJECT_RULES.md` → 본 문서 `Part C / C1 [PF]`

본 문서가 포함된 신규 PassFail 패키지에서는 위 두 구문서를 함께 운영하지 않는다. 과거 패키지의 구문서는 이력 자료로만 취급한다.

사용자가 `XX 기능을 추가해줘`, `이 동작을 수정해줘`, `이 기능을 삭제해줘`처럼 기능 요청만 하더라도 AI는 별도 지시 없이 다음을 자동 적용한다.

- 최신 PassFail Requirements와 사용자 시나리오·FR 확인 및 필요 시 갱신
- LOCK·안전 Gate·충돌·중복·회귀 영향 확인
- 코드 최소 변경과 관련 Regression 수행
- 코드근거·회귀테스트·충돌검토 갱신
- `PassFail_YYMMDD_N.zip` 날짜·순번 자동 산출
- Baseline Revision 동기화 규칙 준수
- Replacement 1안 optional seam과 차량 Stimulus 안전 경계 유지

즉 사용자가 매 작업마다 `Requirements Excel도 갱신해줘`, `패키지 날짜 규칙을 지켜줘`, `개발헌법을 준수해줘`라고 반복 지시할 필요가 없다.
...
| v0.2 / PF v1.3 | 2026-09-12 | PF 최신 패키지 기준 갱신, Governance-only Baseline 정책 명확화, 기존 분리 문서 대체 및 자동 준수 계약을 PF 프로필에 명시 | PF |
