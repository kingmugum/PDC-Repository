# -*- coding: utf-8 -*-
"""
oracle_checker_gui_front_rev87.pyw

Oracle Checker GUI front launcher
- oracle_checker_gui_onebyone_rev87.py : 단일 TC 실행 GUI 본체
- oracle_checker_gui_multiple_rev87.py : 다중 실행 탭/동작 mixin
- oracle_checker_gui_txrx_rev87.py     : 차량 송수신 테스트 탭/진단 mixin (2안, 기존 Injection + Oracle)
- oracle_checker_gui_replacement_rev87.py : 테스트용 P/F 차량 테스트 (1안) 탭 mixin (완전 분리/삭제 가능)
- oracle_checker_replacement_rev87.py  : 1안 Replacement/Inline Gateway 전용 backend (rev87 Dry Run)
- oracle_checker_main_rev87.py         : 판정 core (자동 최신 rev 로드)
- oracle_checker_stimulus_rev87.py     : 실험용 입력 자극 엔진(선택 모듈, TX/RX에서만 호출)
- oracle_checker_collector_rev87.py    : 읽기 전용 CAPL Event Collector 생성/연동(선택 모듈)
...
- 1안 Replacement/Inline Gateway 실험 트랙을 별도 optional 모듈로 추가
  ...
  * rev87 1안은 Dry Run 구조 검증만 제공하고 실제 CAN 차단/forwarding/output은 비활성
...
- 실험용 실제 입력 기능은 oracle_checker_stimulus_rev87.py로 완전 분리
...
from oracle_checker_gui_onebyone_rev87 import OracleCheckerOneByOneGui, CORE_MODULE_PATH
from oracle_checker_gui_multiple_rev87 import MultipleExecutionMixin
from oracle_checker_gui_txrx_rev87 import TxRxTestMixin

# rev87: 1안 Replacement 탭은 완전 optional이다.
# 두 파일을 통째로 삭제해도 fallback mixin이 super()._build_ui()만 호출하므로 기존 앱은 정상 실행한다.
try:
    from oracle_checker_gui_replacement_rev87 import ReplacementVehicleTestMixin
...
def main():
    ...
    def _on_close():
        try:
            # rev87: 창 종료 중 Stimulus가 실행 중이면 먼저 stop_event를 보내
