...
완전 분리 규칙
- "차량 송수신 테스트"(2안) / oracle_checker_gui_txrx_rev85.py를 import하지 않는다.
- oracle_checker_stimulus_rev85.py, main, report, collector도 import하지 않는다.
- 1안 전용 backend인 oracle_checker_replacement_rev85.py만 사용한다.
- 이 GUI 파일과 backend 파일을 삭제하면 front의 optional fallback에 의해 탭만 사라지고 기존 앱은 정상 실행된다.
- rev85은 구조 검증 Dry Run만 제공하고 실제 CAN Gateway/Replacement는 비활성이다.
"""
...
import oracle_checker_replacement_rev85 as _replacement
...
class ReplacementVehicleTestMixin:
    ...
    def _init_replacement_state(self):
        ...
        self.replacement_status_var = tk.StringVar(
            value="1안은 rev85에서 구조 검증 Dry Run 전용입니다. 실제 CAN 차단/Forwarding은 실행하지 않습니다."
        )
    ...
    def _build_replacement_tab(self):
        ...
        ttk.Label(
            ...
            text=(
                ...
                "rev85에서는 실제 Bus를 건드리지 않고, 물리 구조와 전제조건만 검증합니다. "
        ...
        self.replacement_result_text.insert(
            "1.0",
            "[rev85] 1안은 아직 실제 차량 송신/차단 기능을 실행하지 않습니다.\n"
    ...
    def _replacement_validate_dry_run(self):
        ...
        if self.replacement_result_text is not None:
            ...
            self.replacement_result_text.insert(
                ...
                "※ rev85에서는 실제 CAN output()/Gateway forwarding을 호출하는 함수가 없습니다.\n"
            )
    ...
    def _replacement_reset(self):
        ...
        if self.replacement_result_text is not None:
            self.replacement_result_text.delete("1.0", tk.END)
            self.replacement_result_text.insert(
                "1.0", "[rev85] 1안 입력 초기화 완료. Dry Run 검증만 가능합니다.\n"
            )
