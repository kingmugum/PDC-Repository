# -*- coding: utf-8 -*-
"""
oracle_checker_gui_onebyone_rev85.py
...
- rev85:
  ...
  * rev85 Stimulus 실제 입력 로직은 본 모듈에 추가하지 않음
...
class OracleCheckerGui(tk.Tk):
    def __init__(self):
        ...
        # rev85: 고정 1760x930 / 최소 1480x780 강제를 제거한다.
        ...
        # rev85: CAN 네트워크명 근거 기반 DBC 자동 매핑 설정
        ...
        # rev45 도입 / rev85 유지: 다중 TC 수행 전용 로그 재검토 옵션. 대용량 로그 부담을 고려해 기본값은 OFF.
        ...
        # rev85: PassFail은 Vector HW 물리 포트(CH1~CH4)를 관리하지 않는다.
        ...
        # rev85: optional read-only CAPL Event Collector. Default ON; if unavailable it must
        ...
        # rev85: Notebook 각 탭 전체를 양방향으로 탐색하기 위한 공통 scroll registry.
        ...
        # rev85 UI responsiveness / staged progress feedback
        ...
        # rev85: Windows maximize/resize 중 ttk.Notebook이 마지막 추가 탭(1안)으로
    ...
    def _apply_saved_or_auto_defaults(self) -> None:
        ...
        # rev85: 과거 can_ch(물리 채널) 저장값은 의도적으로 무시한다.
    ...
    # -------------------------------------------------
    # rev85 lightweight operation progress popup
    # -------------------------------------------------
    ...
    def _on_notebook_tab_changed(self, _event=None):
        ...
        그 사이에 끼어들 수 없다. 따라서 rev85는 팝업을 *계산 직전* 먼저 그린 뒤 sync를 수행하고
    ...
    def _build_run_tab(self, parent):
        # rev85: 목록 viewport를 좁게 고정하고 내부 X scrollbar로 원래 열 폭을 그대로 탐색한다.
    ...
    def _build_canoe_frame(self, parent):
        ...
        # rev85: 네트워크명 식별 후 DBC 폴더에서 엄격하게 1개 후보만 자동 선택한다.
    ...
    def _load_conditions(self):
        ...
        # rev85: 차량 송수신 탭의 DBC/입출력 분석을 최초 클릭 시점이 아니라
    ...
    def _poll_ui_async_queue(self):
        ...
        if self._conditions_load_thread is not None and self._conditions_load_thread.is_alive():
            ...
            # rev85 TX/RX 사전분석이 58% 이후를 사용하므로 진행률이 다시 55%로 역행하지 않게 한다.
    ...
    def _finish_conditions_load(self, rows, sheet: str, txrx_preload=None):
        try:
            ...
            # rev85: MRO hook들이 기존 state를 초기화한 뒤 TX/RX 선계산 결과를 적용해
    ...
    # -----------------------------
    # rev85: Vector CANoe/CANalyzer COM 연결 현장 진단 (read-only)
    # -----------------------------
    ...
    # -----------------------------
    # rev85 핵심: GetActiveObject 우선 + 기존 프로세스 확인 기반 guarded Dispatch fallback, 제품/버전 하드코딩 금지
    # -----------------------------
    def _connect_canoe(self, show_popup: bool = True):
        ...
        try:
            ...
            # rev85: 연결 정책은 바꾸지 않고, 실패 원인 분석용 정보만 자동 수집한다.
    ...
    # -------------------------------------------------
    # rev85 Optional CAPL Event Collector (read-only observation layer)
    # -------------------------------------------------
    ...
    def _generate_capl_collector(self, show_popup: bool = True):
        if not COLLECTOR_AVAILABLE or collector_module is None:
            msg = "oracle_checker_collector_rev85.py를 사용할 수 없습니다. 기존 COM polling/로그 재검토는 그대로 사용 가능합니다."
        ...
        try:
            ...
            capl_path = self._app_dir / "PF_Observer_Collector_rev85.can"
            manifest_path = self._app_dir / "PF_Observer_Collector_rev85.manifest.json"
            ...
            self._log(f"[COLLECTOR] rev85 CAPL 생성: {capl_path} | {summary}\n")
            if catalog.skipped:
    ...
    def _start_measurement_with_optional_collector(self, conditions, wait_sec: float, label: str):
        ...
        try:
            ...
            if not capl_path.is_file() or not manifest_path.is_file():
                ...
                self._log("[COLLECTOR] 생성된 PF_Observer_Collector_rev85.can/manifest가 없어 runtime binding을 생략합니다.\n")
                return self._safe_start_measurement_for_new_run(wait_sec, label=label)
            try:
    ...
    def _safe_stop_measurement_for_transition(self, label: str, invalidate_after: bool = True) -> bool:
        # rev85: drain latched CAPL events once before Measurement Stop, then keep the Python hit cache
    ...
    def _collect_selected_channels(self) -> List[int]:
        """rev85: DBC가 지정된 CANoe 논리 CAN만 감시 후보로 사용한다."""
        channels = [
    ...
    def _export_report(self):
        ...
        if report_module is None:
            messagebox.showerror(
                ...
                "front/onebyone/multiple 파일과 같은 폴더에 oracle_checker_report_rev85.py를 두세요."
            )
            return
