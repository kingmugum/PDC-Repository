...
- rev85 compact auto-P/F uses a fixed ACTIVE bridge path. The user performs a one-time CANoe Network Node association
...
- rev85 records the intended baseline/active raw payload and cycle time in Stimulus results so post-log diagnostics can compare the requested frame with what the logger actually captured.
- rev85 optional in-worker observer callback lets TX/RX auto-P/F sample output signals while the active frame is held without creating a second COM thread.
- rev85 generic dominance boost: every CAPL stimulus frame is re-sent at one tenth of the DBC nominal cycle during Hold (minimum 10 ms). Example: 200 ms nominal -> 20 ms injection. This improves temporal dominance over an original sender that may remain active, but it does not suppress that sender and does not calculate CRC/Alive/E2E.
"""
...
def coerce_numeric_value(value: Any) -> Any:
    """rev85 PoC accepts numeric signal values only."""
    ...
    try:
        ...
        raise StimulusValidationError(
            f"rev85 입력 테스트는 숫자형 Active Value만 지원합니다: {text}"
        ) from e
...
def generate_vector_capl_bridges(
    ...
    CAPL output(). rev85 can copy generated content into a fixed ACTIVE source and compile already-configured CAPL nodes,
    ...
    for item in items:
        ...
        if length <= 0 or length > 64:
            raise StimulusValidationError(
                f"rev85 CAPL Bridge는 DBC payload 1~64 byte만 지원합니다: {item.message}, DLC/Length={length}"
            )
        ...
        if any(bool(getattr(sig, "is_multiplexer", False)) or getattr(sig, "multiplexer_ids", None) for sig in signals):
            raise StimulusValidationError(
                f"rev85 CAPL Bridge는 multiplexed Message를 지원하지 않습니다: {item.message}"
            )
...
def generate_canalyzer_capl_bridges(
    ...
    """Backward-compatible alias; rev85 bridge can be used by CANoe and CANalyzer."""
...
def capl_function_names_for_group(
    ...
    """Return exact rev85 bridge function names needed for a group without touching Vector COM."""
    ...
    for item in list(profile.candidates or []):
        ...
        length = int(getattr(msg, "length", 0) or 0)
        if length <= 0 or length > 64:
            raise StimulusValidationError(f"rev85 CAPL Bridge는 DBC payload 1~64 byte만 지원합니다: {message}, Length={length}")
        signals = list(getattr(msg, "signals", []) or [])
        if any(bool(getattr(sig, "is_multiplexer", False)) or getattr(sig, "multiplexer_ids", None) for sig in signals):
            raise StimulusValidationError(f"rev85 CAPL Bridge는 multiplexed Message를 지원하지 않습니다: {message}")
...
class StimulusExecutor:
    ...
    def _resolve_dbc_message(self, channel: int, message: str) -> Any:
        ...
        if length <= 0 or length > 64:
            raise StimulusValidationError(
                f"rev85 CAPL Frame backend는 DBC payload 1~64 byte만 지원합니다: {message}, DLC/Length={length}"
            )
        signals = list(getattr(msg, "signals", []) or [])
        if any(bool(getattr(sig, "is_multiplexer", False)) or getattr(sig, "multiplexer_ids", None) for sig in signals):
            raise StimulusValidationError(
                f"rev85 CAPL Frame backend는 multiplexed Message를 지원하지 않습니다: {message}"
            )
        return msg
    ...
    def _get_capl_bridge_functions(
        ...
        if missing:
            raise StimulusValidationError(
                ...
                ". rev85에서는 실제 테스트 worker가 Measurement를 재시작하면서 OnInit에서 GetFunction을 획득해야 합니다."
            )
    ...
    def _prepare_capl_frame(
        ...
        # rev85: Generic temporal-dominance boost for every input stimulus.
    ...
    def validate_group(self, client: Any, profile: StimulusGroupProfile) -> Dict[str, Any]:
        ...
        if len(items) > self.MAX_GROUP_ITEMS:
            raise StimulusValidationError(
                f"rev85 수동 후보 묶음은 최대 {self.MAX_GROUP_ITEMS}개 Signal만 허용합니다. 현재={len(items)}개"
            )
