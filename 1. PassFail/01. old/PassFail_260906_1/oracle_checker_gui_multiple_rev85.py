# -*- coding: utf-8 -*-
"""
oracle_checker_gui_multiple_rev85.py

목적
- Oracle Checker GUI에 "다중 실행" 탭을 추가하기 위한 mixin 모듈
- 단일 실행 GUI 코드는 oracle_checker_gui_onebyone_rev85.py에 유지
...
class MultipleExecutionMixin:
    ...
    def _build_multi_tab(self):
        ...
        # rev85 compact layout: left list is a constrained viewport; internal X scrollbar keeps full columns.
        parent.columnconfigure(0, weight=0, minsize=760)
        # rev85: 우측 전체 영역을 실제로 약 470px(기존 full-width 대비 약 60%)에 고정한다.
        ...
        # rev85: 상태를 작은 화면/Windows theme에서도 한눈에 보이도록 약간 더 선명한 행 음영 사용.
        ...
        # rev85: 내부 위젯의 폭은 대부분 유지하되, 한 줄에 몰아넣지 않고 2줄로 재배치해
        ...
        # rev85: '전체 선택 해제'도 '전체 선택'과 같은 버튼 폭으로 맞춘다.
    ...
    def _on_final_done(self, result):
        ...
        if getattr(self, "multi_active", False) and getattr(self, "multi_mode_var", None) is not None:
            if self.multi_mode_var.get().strip() == "순차실행" and hasattr(self, "multi_sequence_items"):
                ...
                # rev85: 사용자가 [다중 수행 완료]를 눌렀다면 현재 TC 결과 확정 후 전체 순차 세션을 종료한다.
    ...
    def _multi_simultaneous_worker(
        ...
        # rev85: 다중 동시감시는 Excel TC timeout으로 자동 종료하지 않는다.
        ...
        while True:
            ...
            # rev85 optional CAPL Event Collector: consume latched expected-value hits first.
