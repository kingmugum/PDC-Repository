# -*- coding: utf-8 -*-
"""
oracle_checker_gui_txrx_rev85.py

목적
- Oracle Checker GUI에 "차량 송수신 테스트" 탭(2안: Injection + Oracle)을 추가한다.
- rev85 GUI는 전문가용 수동 옵션을 숨기고, 여러 TC 체크 -> 테스트 실행 준비 -> 순차 Stimulus -> 출력 Signal 자동 관측 -> PASS/FAIL로 단순화한다.
...
- rev85: Stimulus 완료 후 최신 BLF/ASC에서 대상 Message의 Tx/Rx 및 Signal 변화, CRC/Alive/Counter 변화를 자동 디버깅 표시한다.
...
- 실험용 입력 1회 테스트만 oracle_checker_stimulus_rev85.py 독립 모듈을 선택적으로 호출
...
- rev85은 회사 Excel 등에서 복사한 후보 Signal 1~8개를 수동 붙여넣기하여 묶음 1회 테스트하는 기능을 추가
...
- Vector CANoe/CANalyzer COM 진단 기능은 공통 onebyone/main rev85에서 제공
- TX/RX 탭은 rev85 공통 Notebook X/Y scroll wrapper를 사용하여 가로/세로 전체 스크롤을 지원한다.
- rev60 전용 TX/RX bind_all은 제거하고 공통 active-tab wheel handler를 사용하여 중복 스크롤을 방지한다.
- rev85 compact UI: 자동화 방식 추천/프리테스트 적합성/수동 실험기록 GUI를 제거하고 PASS/FAIL 열 및 multi-select 자동 P/F를 제공한다.
- rev85 ACTIVE Bridge: CANoe Network Node에 *_ACTIVE.can을 최초 1회 연결하면 이후 bridge 내용 생성/덮어쓰기/CAPL Compile/Measurement binding을 자동화한다.
- rev85 DBC 식별 Bridge: 로컬 DBC 파일명에서 ID를 추출해 `PF_Stimulus_Bridge_CANx_<DBC_ID>_ACTIVE.can`을 자동 생성하고 SHA256을 내장한다. 고정 `CANx_ACTIVE.can`은 패키지 밖 `PF_Stimulus_Runtime` 폴더에 두어 버전 변경 후에도 CANoe 최초 1회 연결을 유지한다.
- rev85 TC별 Measurement Log 이름 자동화: 자동 P/F는 TC마다 Measurement를 명시적으로 Stop하여 CANoe Logging 파일을 닫고, 설정 탭 자동 로그 폴더의 새/변경 ASC·BLF를 `시트_005-3_CAN1_B1_CAN2_M_HHMMSS.ext` 형식으로 rename한다.
"""
...
import oracle_checker_gui_onebyone_rev85 as _onebyone

core = _onebyone.core

# rev85: 실험용 실제 입력 엔진은 선택 모듈이다.
# import 실패가 전체 GUI/기존 PassFail을 깨지 않도록 반드시 optional로 유지한다.
try:
    import oracle_checker_stimulus_rev85 as _stimulus
...
class TxRxTestMixin:
    ...
    def _init_txrx_state(self):
        ...
        # rev85: 실제 자동화 가능성을 한땀한땀 검증하기 위한 실험 기록 상태
        ...
        # rev85 isolated experimental stimulus state
        ...
        # rev85: 현장 Excel에서 후보 Signal을 그대로 복사/붙여넣기하여 시험하기 위한 상태
        ...
        # rev85 CAPL frame backend safety gate: actual injection only after user confirms original sender is isolated/disabled.
        self.txrx_canalyzer_source_isolated_var = tk.BooleanVar(value=False)
        # rev85: CANoe direct Signal.Value requires an Interaction Layer signal driver.
        ...
        # rev85: 하나의 Message/Signal이 CAN1~CAN3 여러 DBC에 존재할 수 있으므로
        ...
        # rev85: DBC 분석 캐시 / 비동기 전체검사 / 수동 후보 검증 캐시
        ...
        # rev85: Stimulus 실행 뒤 최신 BLF/ASC에서 실제 Message/Signal 변화가 기록됐는지 자동 확인한다.
        ...
        # rev85 simplified 2안: multi-select Injection + Oracle auto P/F
        ...
        # rev85: per-TC live/final observed Signal values shown in the second row of the TX/RX tree.
        ...
        # rev85: 자동 Stimulus가 허용되는 물리 작동 TC(예: 시트)는 BLOCK 대신 동적 안전 경고를 표시한다.
    ...
    def _build_txrx_tab(self):
        ...
        # rev85: Windows ttk(Treeview) 일부 theme는 정상/비선택 상태의 style map이
        ...
        self.txrx_tree = ttk.Treeview(
            ...
            show="headings",
            selectmode="none",
            # rev85: expected/actual 2행 구조 기준으로 약 10개 TC(20행)가 한 화면에 보이도록 축소.
            height=20,
            style="TxRx.Treeview",
            takefocus=False,
        )
        ...
        # rev85: the old selected-TC Preview box is removed. The TC list itself now carries
    ...
    def _bind_txrx_mousewheel(self):
        """rev85: 공통 Notebook wheel handler가 모든 탭을 담당한다.
    ...
    # -------------------------------------------------
    # rev85: TC 목록 로드와 함께 TX/RX DBC/입출력 위치 사전 계산
    # -------------------------------------------------
    ...
    def _update_txrx_tree_row(self, idx: int):
        ...
        try:
            ...
            # rev85: native selection/theme state가 tag 색을 덮지 않도록 제거하고 즉시 redraw.
            try:
    ...
    def _on_txrx_tree_select(self, event=None):
        # browse selection is intentionally disabled in rev85 compact tree; Button-1 is authoritative.
        return None
    ...
    def _on_txrx_tree_click(self, event=None):
        ...
        # rev85: 기준행의 체크박스/TC번호/소분류/입력/출력/결과 영역뿐 아니라
        ...
        if shift and self.txrx_auto_anchor_index is not None:
            ...
            if range_indices:
                # rev85: Shift 범위 동작은 anchor의 현재 체크 상태를 범위 전체에 복제한다.
        ...
        # rev85: 일반 클릭은 열 위치와 무관하게 해당 TC의 체크를 즉시 toggle하고
    ...
    # -------------------------------------------------
    # rev85 compact 2안 selection / auto P/F helpers
    # -------------------------------------------------
    def _show_txrx_selection_loading(self, idx: int):
        # rev85: TC 목록 로드 시 이미 분석된 행은 팝업 없이 즉시 Preview/자동 진단 요약을 사용한다.
    ...
    def _txrx_make_auto_plan(self, idx: int, db_by_ch: Dict[int, Any], mapping: Dict[int, str]) -> Dict[str, Any]:
        ...
        if len(inputs) != 1:
            raise ValueError(f"rev85 자동 2안은 입력 조건 1개 TC부터 지원합니다. 현재 입력={len(inputs)}개")
        if not outputs:
    ...
    def _txrx_runtime_bridge_dir(self) -> Path:
        """rev85: package-version-independent runtime directory shared by all PassFail releases.
    ...
    def _txrx_dbc_id_from_path(raw_path: str) -> str:
        """rev85: prefer a short vehicle/DBC identifier (B1/C/M/E/...) from the local DBC filename.
    ...
    def _txrx_stamp_bridge_identity(self, source: Path, logical_can: str, channel: int, dbc_path: str) -> str:
        ...
        marker = "/* PassFail rev85 Bridge Identity */"
        header = "\n".join([
            marker,
    ...
    def _txrx_write_active_bridges(self, plans: List[Dict[str, Any]], db_by_ch: Dict[int, Any], dbc_mapping: Dict[int, str]) -> Tuple[List[str], Any]:
        ...
        group = _stimulus.StimulusGroupProfile(
            name="rev85 Auto P/F Bridge Set", candidates=candidates,
        ...
        generated = _stimulus.generate_vector_capl_bridges(group, resolver, gen_dir, revision="rev85")
        active_paths = []
        ...
        for gp in generated:
            src = Path(gp)
            m = re.search(r"PF_Stimulus_Bridge_(CAN\d+)_rev85\.can$", src.name, re.IGNORECASE)
            if not m:
                continue
    ...
    def _txrx_auto_run_selected(self):
        ...
        try:
            ...
            # rev85: Tk variables are snapshotted on the GUI thread. The worker only uses plain values.
        ...
        def worker():
            ...
            try:
                ...
                for order, p in enumerate(plans, start=1):
                    ...
                    # rev85: one selected TC = one Measurement session = one CANoe logging file boundary.
                    ...
                    # rev85: explicitly close this TC's Measurement before naming the generated CANoe log.
                    self.txrx_async_queue.put((
                ...
                # rev85: each TC already stops Measurement for one-log-per-TC boundaries.
    ...
    def _txrx_begin_session_log(self):
        try:
            ...
            self.txrx_session_log_path = str(log_dir / f"txrx_auto_{stamp}_rev85.log")
            Path(self.txrx_session_log_path).write_text(
                "PassFail rev85 차량 송수신 테스트(2안) 실행/원인 분석 로그\n"
    ...
    def _finish_txrx_stimulus_log_verification(self, report: Dict[str, Any]):
        ...
        self._txrx_log_verify(
            "[LOG-VERIFY][NOTE] rev85 진단은 CAPL 호출 성공, 로그의 실제 Tx/Rx 증거, ECU/차량 기능 반응을 서로 다른 단계로 취급합니다. "
    ...
    def _get_txrx_cached_dbc_map(self):
        """rev85: 동일 DBC를 TC마다 재로딩하지 않고 mapping signature 기준으로 재사용한다."""
        mapping = self._collect_dbc_mapping()
    ...
    def _is_safety_excluded_tc(self, condition: Any) -> bool:
        ...
        rev85부터 짧은 약어는 token 단위로만 판정한다. 시트/등받이/윈도우 등 body-comfort
    ...
    def _analyze_condition_txrx(
        ...
        for pos, exp in enumerate(getattr(condition, "input_conditions", []) or [], start=1):
            ...
            if not locations:
                ...
                # rev85: 복수 CAN 자체는 오류/경고가 아니다. 같은 신호가 여러 DBC에 있으면
        ...
        if result["input_rows"] and not result["errors"]:
            # rev85: 공통 버스 주의문은 정적 적합성 상태를 '확인 필요'로 강등하지 않는다.
    ...
    def _check_txrx_all(self):
        """rev85: 전체 가능성 검사는 GUI thread를 점유하지 않고 DBC를 1회만 로드한다."""
    ...
    def _txrx_or_can_candidates_from_found(self, found: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """rev85: deduplicate/sort logical CAN candidates while preserving each DBC location."""
    ...
    def _txrx_autofill_manual_candidate_from_selected(self, silent: bool = False) -> bool:
        """rev85: selected TC -> one-line manual candidate with CAN1~CAN3 OR-candidate support.
    ...
    def _manual_candidate_logical_to_channel(self, token: str) -> Tuple[str, int]:
        ...
        # rev85 backward compatibility: 과거 붙여넣기 CH1/1도 CAN1로 해석하되 GUI에서는 노출하지 않는다.
    ...
    def _effective_stimulus_hold_sec(self, targets: Optional[List[Tuple[str, str]]] = None) -> float:
        ...
        rev85 changes the displayed default to 2.2 s. Only known seat slide
    ...
    def _parse_manual_stimulus_candidates(self):
        if not STIMULUS_AVAILABLE or _stimulus is None:
            raise RuntimeError(
                "oracle_checker_stimulus_rev85.py를 사용할 수 없습니다. "
        ...
        if len(lines) > _stimulus.StimulusExecutor.MAX_GROUP_ITEMS:
            raise ValueError(
                f"rev85 수동 후보 묶음은 최대 {_stimulus.StimulusExecutor.MAX_GROUP_ITEMS}행만 허용합니다. 현재={len(lines)}행"
            )
        ...
        for lineno, line in enumerate(lines, start=1):
            ...
            if any(k in blob for k in safety_keywords) or any(k in tokens for k in ("str", "tq", "hv")):
                raise ValueError(f"{lineno}행은 rev85 안전핵심 키워드가 포함되어 수동 Stimulus 대상에서 제외합니다: {message}.{signal}")
            candidates.append(
        ...
        return _stimulus.StimulusGroupProfile(
            ...
            candidates=candidates,
            hold_sec=hold_sec,
            source="Manual pasted candidates / rev85",
        )
    ...
    def _require_canalyzer_source_isolated(self, client):
        """Compatibility name: rev85 gate applies to any CAPL Frame backend (CANoe/CANalyzer)."""
    ...
    def _txrx_build_all_input_bridge_profile(self):
        ...
        profile = _stimulus.StimulusGroupProfile(
            name="rev85 Pre-generated Runtime Bridge Set",
            candidates=candidates,
            hold_sec=0.1,
    ...
    def _txrx_prepare_runtime_bridges_without_run(self):
        ...
        try:
            ...
            generated = _stimulus.generate_vector_capl_bridges(profile, resolver, out_dir, revision="rev85")
            channel_by_logical = {}
    ...
    def _txrx_generate_canalyzer_capl_bridge(self):
        ...
        try:
            ...
            out_dir = Path(getattr(self, "_app_dir", Path.cwd())) / "vector_capl_bridge"
            paths = _stimulus.generate_vector_capl_bridges(profile, resolver, out_dir, revision="rev85")
            ...
            messagebox.showinfo(
                ...
                "\n".join([
                    ...
                    *[f"- {x}" for x in identified_paths],
                    "",
                    "rev85은 로컬 DBC 파일명에서 DBC ID를 자동 추출하여 CANx_<DBC_ID>_ACTIVE.can을 생성합니다.",
    ...
    def _prepare_manual_candidate_static_validation(self):
        """rev85 후보 검증: DBC 정적 검사 전용. Vector COM/Measurement/GetFunction을 호출하지 않는다."""
        ...
        if crc_messages and not bool(self.txrx_manual_crc_override_var.get()):
            raise ValueError(
                ...
                "rev85은 이를 자동 계산하지 않습니다. 우선 현재 후보만 시험하려면 "
    ...
    def _txrx_validate_manual_candidates_ui(self):
        try:
            ...
            lines = [
                ...
                "※ rev85 후보 검증은 DBC 정적 검사만 수행합니다. Measurement/COM/CAPL GetFunction은 시작하지 않습니다.",
    ...
    def _txrx_manual_candidate_test_once(self):
        """rev85: 무거운 재검증은 UI thread에서 반복하지 않고, 실제 COM은 worker-owned connection으로 수행한다."""
        ...
        if busy_reason:
            ...
            messagebox.showinfo(
                ...
                "rev85에서는 COM 충돌을 피하기 위해 실제 Stimulus를 동시에 시작하지 않습니다.",
            )
            return
        try:
            profile = self._parse_manual_stimulus_candidates()
        ...
        if cache.get("fingerprint") != fingerprint:
            ...
            messagebox.showwarning(
                ...
                "[후보 검증]을 한 번 실행한 뒤 다시 눌러주세요. rev85에서는 실행 버튼에서 무거운 검증을 다시 반복하지 않습니다.",
            )
            return
        ...
        if crc_messages:
            crc_note = (
                ...
                "rev85는 CRC/Alive를 계산하거나 갱신하지 않으므로 ECU가 이 입력을 무시할 수 있습니다."
            )
    ...
    def _txrx_send_not_implemented(self):
        messagebox.showinfo(
            "일반 송신 비활성",
            "rev85에서도 일반 CAN 송신/로그 Replay 자동화는 활성화하지 않습니다.\n\n"
            "실제 입력 PoC는 별도 oracle_checker_stimulus_rev85.py 모듈의 [입력 1회 테스트] 또는 "
    ...
    def _build_selected_stimulus_profile(self):
        ...
        if self._is_safety_excluded_tc(cond):
            raise ValueError("rev85 Stimulus PoC에서는 제동/조향/구동/에어백/충전/ADAS 등 안전 영향 TC 입력을 차단합니다.")

        inputs = list(getattr(cond, "input_conditions", []) or [])
        if len(inputs) != 1:
            raise ValueError(f"rev85 Stimulus PoC는 입력 조건이 정확히 1개인 TC만 허용합니다. 현재={len(inputs)}개")
        ...
        if row.get("crc_alive"):
            raise ValueError("CRC/Alive/Counter류 Signal이 포함된 Message는 rev85 직접 Signal 입력 대상에서 제외합니다.")
        found = [x for x in (row.get("locations") or []) if x.get("signal_found")]
        candidates = self._set_txrx_or_can_candidates(found)
        if not candidates:
    ...
    def _txrx_stimulus_test_once(self):
        ...
        def worker():
            ...
            measurement_restarted_for_stimulus = False
            try:
                # rev85: pywin32 COM 객체는 생성 스레드에서만 사용한다. 실제 Stimulus worker가 자체 재연결한다.
                worker_client = core.CANoeClient(bus_name=worker_bus_name, product_preference=worker_product).connect(
                    expected_version_keyword=None, expected_exe_keyword=None
                )
                db_by_ch = core.load_dbc_map(mapping) if mapping else {}
    ...
    def _write_txrx_status_text(self, cond: Any, analysis: Dict[str, Any]):
        ...
        lines = [
            ...
            "실제 Stimulus 동작은 독립 oracle_checker_stimulus_rev85.py에서만 수행",
        ]
        if analysis.get("errors"):
            lines.append("")
            lines.append("[불가 사유]")
