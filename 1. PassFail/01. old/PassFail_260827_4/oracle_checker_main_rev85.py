...
- rev85:
  ...
  * 실제 입력 PoC는 독립 oracle_checker_stimulus_rev85.py에서만 수행
  * DispatchEx/Registry 쓰기/무조건적 새 인스턴스 생성은 금지. Dispatch는 기존 제품 프로세스 확인 시 guarded fallback으로만 사용
  * 기존 DBC 자동매핑/COM 진단/판정 계약은 유지
  * rev85 TX/RX Stimulus 진단: per-run 분석 window, 전체/채널 Tx 기록 능력, 동일-ID Rx 지속, intended payload diff, TC 출력 Expected 반응을 BLF/ASC에서 분리 분석
  * rev85 추가 진단: intended baseline과 같은 실행창의 실제 RX raw frame byte diff를 표시하여 재구성 baseline 불일치를 검출
"""
...
# =========================================================
# Vector CANoe/CANalyzer COM read-only diagnostics (rev85)
# =========================================================
...
def _build_vector_diag_interpretation(diag: Dict[str, Any]) -> List[str]:
    ...
    if active.get("success"):
        ...
        notes.append("Vector 프로세스는 감지되지만 GetActiveObject가 실패했습니다. rev85 실제 연결은 제품별 메인 프로세스가 사전 확인된 경우에만 guarded Dispatch fallback을 1회 시도합니다. 수동 진단 자체는 read-only이며 fallback을 실행하지 않습니다.")
    else:
...
# =========================================================
# CAN network name / DBC auto mapping utilities (rev85)
# =========================================================
...
class VectorApplicationClient:
    ...
    def __init__(self, bus_name: str = "CAN", product_preference: Any = "Auto"):
        ...
        self._app_fullname = ""
        self._cfg_fullname = ""
        # rev85: CAPLFunction objects must be acquired during Measurement.OnInit.
    ...
    def connect(
        ...
        if probe.get("success"):
            ...
            # rev85 guarded fallback: only when a product-specific main process already exists.
    ...
    def compile_existing_capl_nodes(self) -> None:
        ...
        rev85 ACTIVE-bridge workflow intentionally does not create/insert Simulation Setup nodes.
        ...
        if "PassFail_" in detail and "PF_Stimulus_Bridge_" in detail and "ACTIVE.can" in detail:
            migration_hint = (
                "\n[rev85 경로 마이그레이션] 현재 CANoe/CANalyzer Network Node가 과거 PassFail 패키지 폴더의 "
                "PF_Stimulus_Bridge_CANx_ACTIVE.can을 참조하는 것으로 보입니다. "
                "rev85부터는 PassFail 상위 폴더의 PF_Stimulus_Runtime/PF_Stimulus_Bridge_CANx_ACTIVE.can을 "
    ...
    def start_measurement_with_capl_bindings(
        ...
        Measurement.OnInit. rev85 uses this helper for isolated TX/RX Stimulus and the optional
...
def inspect_stimulus_transitions_in_log(
    ...
    rev85 deliberately separates these layers:
...
def wait_for_expectation_realtime(
    ...
    while True:
        ...
        # rev85 optional CAPL Event Collector: CAPL latches a short expected-value edge inside
