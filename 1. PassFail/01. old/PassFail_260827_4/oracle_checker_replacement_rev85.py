...
- rev85에서는 구조/전제조건 검증(Dry Run)만 제공하며 실제 CAN 송신/차단/forwarding은 수행하지 않는다.
- 이 파일과 oracle_checker_gui_replacement_rev85.py를 삭제해도 기존 PassFail은 정상 동작해야 한다.
"""
...
def validate_replacement_plan(plan: ReplacementPlan) -> List[ReplacementCheck]:
    ...
    checks.append(ReplacementCheck(
        "INFO", "rev85 실행 범위",
        "rev85 1안 모듈은 Dry Run 전용입니다. 실제 CAN 차단/forwarding/output은 의도적으로 구현하지 않았습니다."
    ))
    return checks
...
def summarize_replacement_checks(checks: List[ReplacementCheck]) -> str:
    ...
    return "구조 검증: BLOCK 0 / WARN 0 — 구조 전제 확인됨 (단, rev85은 Dry Run 전용)"
